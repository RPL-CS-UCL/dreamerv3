...
class Dreamer(nn.Module):
    ...
    def __call__(self, obs, reset, state=None, training=True):
        ...
        #print("return from dreamer call ", policy_output, state)
...
def main(config):
    ...
    print("** steps",  count_steps(config.traindir))
    print("Create envs.")
    ...
    if not config.offline_traindir:
        prefill = config.prefill# - count_steps(config.traindir))
    ...
    agent = Dreamer(
        train_envs[0].observation_space, # remapped for our env
    ...
    while agent._step < config.steps + config.eval_every:
        ...
        state = tools.simulate(
            agent, #dreamer agent
            train_envs, #gym env
            train_eps, # used for cache - replay?
            config.traindir,
            logger,
            limit=config.dataset_size,
